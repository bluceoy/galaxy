...
class CustomJobsAPIController(BaseAPIController):
  ...
  def __on_mst(self, trans, payload, **kwargs):
    ...
    agent_cwd = trans.app.config.tool_path + "/custom"
    job_cwd = trans.app.config.tool_path + "/MST"
    ...
    params = {
      ...
      "galaxy_version": trans.app.config.version_major,
      "cwd": job_cwd,
      "params": "%s %s %s %s %s %s" % (input1, input2, input3, input4, output1, output2),
      "session_id": trans.galaxy_session.id,
      "user_id": user_id,
      "status": 1,
  ...
  def __on_argpore(self, trans, payload, **kwargs):
    ...
    params = {
      ...
      "galaxy_version": trans.app.config.version_major,
      "cwd": job_cwd,
      "params": "%s %s %s %s %s %s" % (input1, output1, output2, output3, output4),
      "session_id": trans.galaxy_session.id,
      "user_id": user_id,
      "status": 1,
      "output": input1_dir + "/output/output1.txt",
  ...
  def on_job_detail(self, trans, id, **kwargs):
    ...
    * GET /api/custom/job/detail/{job_id}
    ...
    output = [ job["output"] ]
    if not job["output2"]:
      log.info("output2 = ", job["output2"])
      output.append(job["output2"])
    if not job["output3"]:
      log.info("output3 = ", job["output3"])
      output.append(job["output3"])
    if not job["output4"]:
      log.info("output4 = ", job["output4"])
      output.append(job["output4"])
    if not job["output5"]:
      log.info("output5 = ", job["output5"])
